...
type CodexUsageSummary = {
  ...
  fiveHourResetAfter: number | null
  weeklyPercent: number | null
  weeklyResetAfter: number | null
}
...
function resolveCodexUsageSummary(
  ...
  const planType = String(payload.plan_type ?? rateLimit.plan_type ?? '')
    .trim()
    .toLowerCase()
  ...
  for (const windowData of windows) {
    const duration = toFiniteNumber(windowData.limit_window_seconds)
  ...
  if (planType === 'free') {
    ...
    fiveHourWindow ??= windows.find((item) => item !== weeklyWindow) ?? null
    weeklyWindow ??= windows.find((item) => item !== fiveHourWindow) ?? null
  }

  return {
    planType,
    fiveHourPercent: toFiniteNumber(fiveHourWindow?.used_percent),
    fiveHourResetAfter: toFiniteNumber(fiveHourWindow?.reset_after_seconds),
    weeklyPercent: toFiniteNumber(weeklyWindow?.used_percent),
    weeklyResetAfter: toFiniteNumber(weeklyWindow?.reset_after_seconds),
  }
}

function formatCodexUsagePercent(value: number): string {
  return `${Math.max(0, Math.min(100, value)).toFixed(0)}%`
}

function formatCodexUsageDuration(seconds: number): string {
...
function BalanceCell({ channel }: { channel: Channel }) {
  ...
  const codexUsageSummary = resolveCodexUsageSummary(codexUsageResponse)
  ...
  useEffect(() => {
    if (isTagRow || channel.type !== 57) {
      setCodexUsageResponse(null)
      return
    }

    let cancelled = false
    void getCodexUsage(channel.id)
      .then((response) => {
        if (!cancelled && response.success) {
          setCodexUsageResponse(response)
        }
      })
      .catch(() => {
        // Automatic usage loading is best-effort and must not interrupt the table.
      })

    return () => {
      cancelled = true
    }
  }, [channel.id, channel.type, isTagRow])
  ...
  if (sensitiveVisible && isUpdating) {
    ...
    const usageLabels = [
      codexUsageSummary?.fiveHourPercent,
      codexUsageSummary?.weeklyPercent,
    ]
      .filter((value): value is number => value !== null && value !== undefined)
      .map(formatCodexUsagePercent)
    remainingBadgeLabel =
      usageLabels.length > 0 ? usageLabels.join('/') : t('Account Info')
  }
    ...
    <TooltipProvider>
      <div className='-ml-1.5 flex items-center gap-1'>
        ...
        <Tooltip>
          ...
          />
          <TooltipContent>
            {channel.type === 57 && sensitiveVisible && codexUsageSummary ? (
              <div className='space-y-1'>
                {codexUsageSummary.planType && (
                  <p>
                    {t('Plan')}: {codexUsageSummary.planType.toUpperCase()}
                  </p>
                )}
                {codexUsageSummary.fiveHourPercent !== null && (
                  <p>
                    {t('5-Hour Window')}:{' '}
                    {codexUsageSummary.fiveHourPercent.toFixed(1)}%
                    {codexUsageSummary.fiveHourResetAfter !== null &&
                      codexUsageSummary.fiveHourResetAfter > 0 &&
                      ` · ${t('Resets in:')} ${formatCodexUsageDuration(codexUsageSummary.fiveHourResetAfter)}`}
                  </p>
                )}
                {codexUsageSummary.weeklyPercent !== null && (
                  <p>
                    {t('Weekly Window')}:{' '}
                    {codexUsageSummary.weeklyPercent.toFixed(1)}%
                    {codexUsageSummary.weeklyResetAfter !== null &&
                      codexUsageSummary.weeklyResetAfter > 0 &&
                      ` · ${t('Resets in:')} ${formatCodexUsageDuration(codexUsageSummary.weeklyResetAfter)}`}
                  </p>
                )}
